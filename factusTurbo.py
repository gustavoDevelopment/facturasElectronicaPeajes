...
def extraer_datos_factura(xml_file):
    ...
    texto = (
            ...
            f"💰 Total: {valor_total} {moneda}\n"
            f"🏢 Proveedor: {proveedor_nombre} (NIT: {proveedor_nit})\n"
            f"👤 Cliente: {cliente_nombre} (NIT: {cliente_nit})\n"
            f"🧾 Peaje: {nombre_peaje}\n"
            f"🚗 Placa: {numero_placa}\n\n"
            f"📦 Ítems:\n" + "\n".join(lineas)
    )
   
    return factura_id, texto
